...
class FileContentsManager(FileManagerMixin, ContentsManager):
    ...
    def _notebook_model(self, path, content=True):
        ...
        if content:
            self.log.debug('About to validate in filemanager.py _notebook_model()')
    ...
    def save(self, model, path=''):
        ...
        try:
            if model['type'] == 'notebook':
                self.log.debug('entering filemanager.py save() for notebook')
        ...
        if model['type'] == 'notebook':
            self.log.debug('Second validation check in filmanager.py save()')
            self.validate_notebook_model(model)
